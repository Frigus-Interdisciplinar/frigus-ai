# Memória incremental de fatos (Fase 2 do PENDENCIAS.md) - Arquivado
...
- **Injeção de fatos estruturados no prompt do chat**: `load_prompt` em
  `graph/prompts/__init__.py` agora aceita parâmetro `fatos` opcional. O
  `dynamic_prompt` middleware em `graph/agents.py` extrai `user_id` do contexto da
  requisição e busca os fatos no Mongo, incluindo-os no system prompt do orquestrador
  e de outros agentes. Isso permite que o LLM use restrições, alergias, preferências
  e hábitos na personalização da resposta em tempo real.
...
`src/frigus_ai/graph/prompts/__init__.py`,
`src/frigus_ai/graph/agents.py`,
`src/frigus_ai/repositories/fatos_repository.py`,
`src/frigus_ai/services/chat_service.py`,
`src/frigus_ai/services/user_service.py`,
`src/frigus_ai/schemas/models.py`,
`src/frigus_ai/api/routes/profile.py`,
`src/frigus_ai/graph/prompts/fatos.md`.