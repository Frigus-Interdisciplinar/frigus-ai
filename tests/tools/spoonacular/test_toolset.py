...
_tools = core.SpoonacularToolSet().as_tools()
find_recipes_by_ingredients = _tools[0]
get_recipe_information = _tools[1]
...
def _fake(monkeypatch, json_data, status_code=200):
    ...
    monkeypatch.setattr(core.spoonacular, "connect", lambda: fake)
    return fake
...
def test_find_recipes_mapeia_resultado_e_params(monkeypatch):
    ...
    r = find_recipes_by_ingredients.invoke(
...
def test_get_recipe_information_traduz_aliases(monkeypatch):
    ...
    r = get_recipe_information.invoke({"recipe_id": 42})
...
def test_cache_evita_segunda_chamada(monkeypatch):
    ...
    find_recipes_by_ingredients.invoke({"ingredients": ["ovo"]})
    find_recipes_by_ingredients.invoke({"ingredients": ["ovo"]})
