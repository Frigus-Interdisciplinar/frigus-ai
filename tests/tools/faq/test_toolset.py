from frigus_ai.graph.tools.faq import toolset as core

faq_retriever = core.FaqToolSet().as_tools()[0]
...
def test_faq_retriever_retorna_resultados(monkeypatch):
    ...
    resultado = faq_retriever.invoke({"question": "como funciona o estoque?"})
...
def test_faq_retriever_sem_resultados(monkeypatch):
    ...
    resultado = faq_retriever.invoke({"question": "pergunta sem match"})
...
def test_faq_retriever_erro_vira_response_error(monkeypatch):
    ...
    resultado = faq_retriever.invoke({"question": "qualquer coisa"})
